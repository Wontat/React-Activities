...
modifier onlyOwner() {
require(msg.sender == owner, "Only the owner has the control to this function");
_;
}
...
function setPrelimGrade(uint256 grade, address studentAddress) external onlyOwner ValidGrade(grade) {
students[studentAddress].prelimGrade = grade;
}

function setMidtermGrade(uint256 grade, address studentAddress) external onlyOwner ValidGrade(grade) {
students[studentAddress].midtermGrade = grade;
}

function setFinalGrade(uint256 grade, address studentAddress) external onlyOwner ValidGrade(grade) {
